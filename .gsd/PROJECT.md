...
M003 has started, and S01 is now the completed benchmark substrate for the rest of the milestone. The repo now includes a normalized acquisition benchmark service, operator-facing benchmark CLI/report commands, a preserve-live VPS experiment runner that auto-detects the usable remote Python interpreter, and durable benchmark artifacts under `.gsd/milestones/M003/benchmarks/` including the FR `page_limit=1` baseline bundle that future transport/frontier/lane slices can compare mechanically.

M003/S02 is now complete. The repo and the live VPS both carry the lane-aware runtime closeout fixes: benchmark-triggered multi-lane windows force an immediate start instead of inheriting a far-future live `next_resume_at`, top-level runtime truth aggregates from the active lane views, the `dual-lane-smoke` runner no longer leaves remote snapshot churn in the multi-lane path, and the final canonical 30-minute proof bundle now exists at `.gsd/milestones/M003/benchmarks/dual-lane-smoke.json` / `.md` with `serving_verification.ok == true`, two completed `frontier` cycles, and two completed `expansion` cycles on the live VPS.
...
- `python -m pytest tests/test_acquisition_benchmark.py tests/test_acquisition_benchmark_cli.py tests/test_vps_benchmark_runner.py -q` passes with **12 passed**, proving the M003/S01 benchmark scorecard contract, redacted CLI/report surfaces, VPS-runner-bundle re-rendering, remote-Python auto-detection, preserve-live VPS runner path, and durable bundle writing.
- `python scripts/run_vps_benchmark.py --host 46.225.113.129 --profile baseline-fr-page1 --duration-minutes 2 --output .gsd/milestones/M003/benchmarks/_probe-baseline-fr-page1.json --markdown .gsd/milestones/M003/benchmarks/_probe-baseline-fr-page1.md` produces a fresh preserve-live VPS benchmark probe with three successful bounded cycles, a copied SQLite snapshot, resource/storage evidence, and a benchmark leaderboard rooted in the real FR `page_limit=1` profile.
- `python -m vinted_radar.cli acquisition-benchmark-report --input .gsd/milestones/M003/benchmarks/_closeout-baseline-fr-page1.json --format markdown` now re-renders the saved VPS runner bundle directly, proving the operator-facing report path works on the same artifact shape emitted by `scripts/run_vps_benchmark.py`.
- `python -m pytest tests/test_runtime_repository.py tests/test_runtime_service.py tests/test_runtime_cli.py tests/test_dashboard.py tests/test_vps_benchmark_runner.py tests/test_http.py -q` passes with **67 passed**, covering the S02 closeout fixes for immediate benchmark lane starts, aggregated top-level runtime truth, serving-verifier transition tolerance, lane-aware CLI/dashboard surfaces, and the repaired VPS benchmark runner.
- `python scripts/run_vps_benchmark.py --host 46.225.113.129 --profile dual-lane-smoke --duration-minutes 30 --verify-base-url http://46.225.113.129:8765 --output .gsd/milestones/M003/benchmarks/dual-lane-smoke.json --markdown .gsd/milestones/M003/benchmarks/dual-lane-smoke.md` now produces the final canonical S02 VPS proof bundle with `remote_result.ok == true`, `serving_verification.ok == true`, both `frontier` and `expansion` present in `lane_results`, and two completed cycles per lane over the real live-VPS window `2026-04-10T08:51:06+00:00` → `2026-04-10T09:06:37+00:00`.
- `python - <<'PY' ... dual-lane bundle + serving assertions ... PY` validates the canonical bundle and confirms `/runtime`, `/api/runtime`, and `/health` all return HTTP 200 after the proof run restores the normal VPS service posture.
