...
M002/S10, M002/S11, M002/S12, M002/S13, and M002/S14 are now complete. The repo has the staged polyglot platform foundation from S10, the immutable-evidence path from S11, the first real PostgreSQL mutable-truth path from S12, the first ClickHouse serving-warehouse path from S13, and now the historical backfill plus live application cutover path from S14: resumable SQLite-to-platform backfill, cross-store reconciliation, explicit cutover diagnostics, ClickHouse + PostgreSQL-backed product/runtime reads, and a documented cutover/rollback smoke path. SQLite no longer needs to remain the intended live product/control-plane history boundary; it now serves as the fallback/shadow read path during staged rollout plus the historical migration input. S15 remains to add retention, reconciliation closure, and AI-ready marts on top of the new platform.
...
- `python3 -m pytest tests/test_cutover_smoke.py -q` exits **0 with 1 skipped** in this shell because Docker is unavailable, proving the shipped cutover smoke harness is wired correctly while the full container-backed proof remains environment-gated
...
- M002 still needs S15 to add retention plus AI-ready marts on top of the new Parquet/object-store evidence boundary
...
- [ ] M002: Enriched Market Intelligence Experience — product and platform slices S01 through S14 are complete. The milestone remains open for S15 to add retention plus AI-ready marts on top of the new Parquet/object-store evidence layer.
